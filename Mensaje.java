...
public class Mensaje
{
    ...
    {   
        ...
        //La linea debe estar compuesta por solo caracteres ascii.
        if (!esAscii(linea)) {
            throw new IllegalArgumentException("La linea solo puede contener caracteres ascii.");
        }
        //La linea no debe pasar la longitud máxima de 80 caracteres.
        if (linea.length() > LONG_MAX_LINEA) {
            throw new IllegalArgumentException("El argumento (linea) no debe contener más de 80 caracteres.");
        }
        //La posición debe ser un valor válido.
        if (pos < 0 || pos > cantLineas()) {
            throw new IllegalArgumentException("La posición ingresada deber estar entre el 0 y el limite de la lista.");
        }
        ...
        assert repOK();
        //Trae el index de la posición donde agregamos la linea y lo compara que sea la misma linea ingresada.
